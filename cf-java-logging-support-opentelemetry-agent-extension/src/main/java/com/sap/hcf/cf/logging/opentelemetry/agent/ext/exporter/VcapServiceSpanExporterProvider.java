...
public class VcapServiceSpanExporterProvider implements ConfigurableSpanExporterProvider {

    private static final Logger LOG = Logger.getLogger(VcapServiceSpanExporterProvider.class.getName());
    private static final String MSK_SKIP = " Skipping span exporter registration.";
    ...
    public VcapServiceSpanExporterProvider() {
        this(config -> new VcapServiceProvider(config).get(),
             config -> VcapServiceCredentials.parser(config, RUNTIME.CLOUD_FOUNDRY.SERVICE.VCAP_SERVICE.CREDENTIALS.OTLP_TRACES_ENDPOINT),
             new ServerCertificateDownloader());
    }

    VcapServiceSpanExporterProvider(Function<ConfigProperties, CloudFoundryServiceInstance> serviceProvider,
    ...
    private static void addTimeout(Consumer<Duration> setter, ConfigProperties config) {
        Duration timeout = EXPORTER.VCAP_SERVICE.TRACES.TIMEOUT.getValue(config);
        if (timeout != null) {
    ...
    public SpanExporter createExporter(ConfigProperties config) {
        ...
        switch (EXPORTER.VCAP_SERVICE.TRACES.PROTOCOL.getValue(config)) {
        case "http/protobuf":
            return createHttpProtobufSpanExporter(config, credentials, serviceInstance);
        case "grpc":
            return createGrpcSpanExporter(config, credentials, serviceInstance);
        default:
            LOG.warning(format(MSG_UNSUPPORTED_PROTOCOL, EXPORTER.VCAP_SERVICE.TRACES.PROTOCOL.getValue(config),
                               serviceInstance.getName(), serviceInstance.getLabel()));
        }
        return NoopSpanExporter.getInstance();
    }

    private SpanExporter createGrpcSpanExporter(ConfigProperties config, VcapServiceCredentials credentials,
                                                CloudFoundryServiceInstance serviceInstance) {
        OtlpGrpcSpanExporterBuilder builder =
                OtlpGrpcSpanExporter.builder().setEndpoint(credentials.getEndpoint())
                                    .setCompression(EXPORTER.VCAP_SERVICE.TRACES.COMPRESSION.getValue(config))
    ...
    private SpanExporter createHttpProtobufSpanExporter(ConfigProperties config, VcapServiceCredentials credentials,
                                                        CloudFoundryServiceInstance serviceInstance) {
        OtlpHttpSpanExporterBuilder builder =
                OtlpHttpSpanExporter.builder().setEndpoint(credentials.getEndpoint())
                                    .setCompression(EXPORTER.VCAP_SERVICE.TRACES.COMPRESSION.getValue(config))
                                    .setRetryPolicy(RetryPolicy.getDefault());
        addTimeout(builder::setTimeout, config);
        if (!addTlsConfig(builder::setClientTls, builder::setTrustedCertificates, credentials,
